...
function refresh() {
  if (dealSum > 21) {
    ...
    removeImg();
    doubleChips = (bet * 6) + debitChips;
    document.querySelector('#chipCountResult').innerHTML = "Rp." + doubleChips;
  ...
  if (debitChips === 0) {
    document.querySelector('#noChipMsg').innerHTML = "<==|| UANG ANDA HABIS ||"
    document.querySelector('#dealbutton').disabled = true;
  }
  bet = 0;
  plySum = 0;
...
function chips() {
  while (bet <= 0 || bet >= (debitChips + 1)) {
    bet = Math.round(prompt('Berapa Banyak Uang Yang Ingin Anda Taruhkan?'));
  }
  debitChips = (debitChips - bet);
  document.querySelector('#chipCountResult').innerHTML = "Rp." + debitChips;
}
